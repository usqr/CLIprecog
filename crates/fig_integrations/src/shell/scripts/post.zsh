...
fig_precmd() {
  ...
  fig_osc "FigAutosuggestionColor=%s" "${KIRO_AUTOSUGGEST_HIGHLIGHT_STYLE}"
  fig_osc "User=%s" "${USER:-root}"

  if [ "$KIRO_HAS_SET_PROMPT" -eq 1 ]; then
  ...
  KIRO_USER_PS1="$PS1"
  KIRO_USER_PROMPT="$PROMPT"
  KIRO_USER_prompt="$prompt"

  KIRO_USER_PS2="$PS2"
  KIRO_USER_PROMPT2="$PROMPT2"

  KIRO_USER_PS3="$PS3"
  KIRO_USER_PROMPT3="$PROMPT3"

  KIRO_USER_PS4="$PS4"
  KIRO_USER_PROMPT4="$PROMPT4"

  KIRO_USER_RPS1="$RPS1"
  KIRO_USER_RPROMPT="$RPROMPT"

  KIRO_USER_RPS2="$RPS2"
  KIRO_USER_RPROMPT2="$RPROMPT2"
