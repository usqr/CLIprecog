...
_{{CLI_BINARY_NAME_UNDERSCORE}}_autosuggest_strategy_completion() {
	...
	if zle; then
		zpty $Q_AUTOSUGGEST_COMPLETIONS_PTY_NAME _{{CLI_BINARY_NAME_UNDERSCORE}}_autosuggest_capture_completion_sync
	else
		zpty $Q_AUTOSUGGEST_COMPLETIONS_PTY_NAME _{{CLI_BINARY_NAME_UNDERSCORE}}_autosuggest_capture_completion_async "\$1"
