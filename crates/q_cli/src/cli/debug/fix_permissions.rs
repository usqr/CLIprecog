...
pub fn fix_permissions(ctx: &fig_os_shim::Context) -> Result<()> {
    ...
    for shell in Shell::all() {
        let shell_dir = shell.get_config_directory(ctx)?;
        ...
        if let Ok(integrations) = shell.get_shell_integrations(ctx) {
